...
and share-manager health first. Replace affected workers only through
controller-owned recovery and verify committed hashes. Never interpret Service
readiness as recovery of an existing NFS client, replay a turn/effect, or claim
seamless failover. Longhorn's recovery-backend policy remains restricted to
pods labeled `longhorn.io/component=share-manager`; HOR-527 must establish the
actual failure cause before proposing any exception to that selector.
