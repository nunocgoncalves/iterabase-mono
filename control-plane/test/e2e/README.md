...
Required source and candidate modes both consume the runtime manifest produced by `.github/scripts/e2e.py compose`. Artifact builders run before owner scenarios and build each affected image/chart only once from the exact source SHA using the reviewed release recipe. The composer resolves selected temporary or candidate images plus only explicit immutable baselines to exact archives/references, composes the exact platform/substrate chart pair, and exports the same `CONTROL_PLANE`, `HARNESS`, `TOOL_RUNNER`, `INFERENCE_GATEWAY`, and deterministic-runtime identities in both modes.

Owner stages never build artifacts or choose a mode-specific DAG. Every scenario creates Kind, runs `import-runtime-images` to restore each composer archive, verify its config digest/source label, and transport its exact reference into the new cluster, then executes the same installs/assertions. Deployed identity checks use the imported single-platform manifest digest returned by Kind/CRI; they do not confuse it with the distinct config or registry/index digest. A runner-daemon load before cluster creation cannot satisfy this stage. Required execution also writes one strict result record containing every declared stage and observed artifact identity; a skip, blocked/not-run stage, missing report, or identity mismatch fails the aggregate. Published mode remains local compatibility-only and is not a PR/nightly/candidate route.
