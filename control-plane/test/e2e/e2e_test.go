...
func deployedBrowserJourneysScenario() sharede2e.Definition {
	...
	return sharede2e.Define(sharede2e.Scenario[*deployedState]{
		...
		Stages: []sharede2e.Stage[*deployedState]{
			...
			{Name: "configure-agentpool-local-path", DependsOn: []string{"install-control-plane"}, Run: configureKindWorkspaceStorage},
			{Name: "assert-deployment-ready", DependsOn: []string{"configure-agentpool-local-path"}, Run: assertDeploymentReadyStage},
...
func deployedExecutionContractsScenario() sharede2e.Definition {
	...
	return sharede2e.Define(sharede2e.Scenario[*deployedState]{
		...
		Stages: []sharede2e.Stage[*deployedState]{
			...
			{Name: "configure-agentpool-local-path", DependsOn: []string{"install-execution-platform"}, Run: configureKindWorkspaceStorage},
			{Name: "assert-execution-platform-ready", DependsOn: []string{"configure-agentpool-local-path"}, Run: assertExecutionPlatformReadyStage},
