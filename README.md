...
### Artifact backup, restore, and deletion runbook

Postgres metadata and MinIO bytes are one coordinated recovery unit. Run these
commands from an approved operator host with `pg_dump`/`pg_restore`, `psql`, and
MinIO `mc` available. Supply credentials through the operator's secret manager;
do not persist them in this repository or a sandbox.

```sh
export BACKUP_DIR="/secure/iterabase-backups/$(date -u +%Y%m%dT%H%M%SZ)"
export DATABASE_URL='postgresql://...'
export RESTORE_DATABASE_URL='postgresql://.../iterabase_restore'
export MINIO_ALIAS=opo1
export RESTORE_MINIO_ALIAS=restore
export ARTIFACT_BUCKET=iterabase-artifacts
mkdir -p "$BACKUP_DIR/minio"
```

1. **Quiesce writes.** Put the installation in maintenance mode and scale every
   API/gateway instance that can upload, publish, retain, or delete artifacts to
   zero. Record the deployment names and replica counts so they can be restored.
   Do not continue while an artifact writer or lifecycle sweeper is running.
2. **Back up metadata, then bytes while still quiesced.** This ordering records
   the authoritative metadata first and leaves writers stopped until both
   copies are complete.
   ...
   mc mirror --overwrite --remove \
     "$MINIO_ALIAS/$ARTIFACT_BUCKET" \
     "$BACKUP_DIR/minio/$ARTIFACT_BUCKET"
   find "$BACKUP_DIR" -type f ! -name BACKUP.sha256 -print0 | sort -z | xargs -0 sha256sum \
     > "$BACKUP_DIR/BACKUP.sha256"
   ```

3. **Restore into an isolated target with services still stopped.** Restore
   MinIO first and Postgres second, so available metadata is never exposed
   before its bytes exist.

   ```sh
   (cd "$BACKUP_DIR" && sha256sum --check BACKUP.sha256)
   mc mb --ignore-existing "$RESTORE_MINIO_ALIAS/$ARTIFACT_BUCKET"
   mc mirror --overwrite --remove \
     "$BACKUP_DIR/minio/$ARTIFACT_BUCKET" \
     "$RESTORE_MINIO_ALIAS/$ARTIFACT_BUCKET"
   pg_restore --clean --if-exists --no-owner \
     --dbname "$RESTORE_DATABASE_URL" "$BACKUP_DIR/postgres.dump"
   ```

4. **Verify every available artifact against durable metadata.** The restored
   service credential must be bucket-scoped. Any missing object, size mismatch,
   or digest mismatch fails the restore; do not start API/gateway replicas.

   ```sh
   psql "$RESTORE_DATABASE_URL" -At -F '|' -c \
     "SELECT storage_key,size_bytes,digest FROM artifact.artifacts WHERE state='available' ORDER BY id" |
   while IFS='|' read -r key expected_size expected_digest; do
     actual_size=$(mc stat --json "$RESTORE_MINIO_ALIAS/$ARTIFACT_BUCKET/$key" | jq -r .size)
     actual_digest="sha256:$(mc cat "$RESTORE_MINIO_ALIAS/$ARTIFACT_BUCKET/$key" | sha256sum | cut -d' ' -f1)"
     test "$actual_size" = "$expected_size" && test "$actual_digest" = "$expected_digest" || exit 1
   done
   ```

5. **Validate explicit deletion before ending maintenance.** Using an admin
   bearer token, delete a disposable restored artifact and verify both the
   retained tombstone and absent object. A transient object-delete failure must
   leave `state='deleting'`/`deletion_error` for the sweeper to retry.

   ```sh
   curl --fail-with-body -X DELETE \
     -H "Authorization: Bearer $ITERABASE_ADMIN_TOKEN" \
     "$ITERABASE_API_URL/v1/artifacts/$ARTIFACT_ID"
   psql "$RESTORE_DATABASE_URL" -x -c \
     "SELECT state,deleted_at,deletion_reason,deletion_error FROM artifact.artifacts WHERE id='$ARTIFACT_ID'"
   ! mc stat "$RESTORE_MINIO_ALIAS/$ARTIFACT_BUCKET/artifacts/$ARTIFACT_ID"
   ```

Capture the UTC window, Git/Flux revision, Postgres dump checksum, MinIO mirror
summary, per-artifact digest result, deletion result, and operator identity in
the OPO1 release evidence. Automated repository validation covers MinIO
round-trip/integrity/deletion and retryable pending cleanup; the live OPO1
backup/restore/digest transcript remains a deployment acceptance gate and must
be attached to HOR-399 before release acceptance.
