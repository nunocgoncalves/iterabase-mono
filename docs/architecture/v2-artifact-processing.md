...
The founder approved the complete package below for HOR-460 on 2026-08-23. The full approval and consequences are also durable in the Linear issue. A product-behavior change or a different inherited datastore, component, transport, isolation, failure, or lifecycle model requires a new durable decision rather than an implementation-local interpretation.
...
- ClamAV engine/config/signature version capture; fresh/current/stale/corrupt/missing signature readiness; updater source/mirror failure and validated rollback.
...
| Open architecture choices resolved | `DES-HOR-460-01`–`12`; future LLM/component/format/storage changes require separate decisions. |
