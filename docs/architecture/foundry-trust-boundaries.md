...
| `SCN-FND-RSCH-001` | A bounded campaign runs in an isolated lane, cannot access hidden qualification, and cannot self-promote (Items 3, 8, 11); exact mutation allowlist, pinned controls, and budget/plateau-stop mechanics remain HOR-524/HOR-532. |
...
| `SCN-FND-ACC-001` | The exact deployed configuration produces an immutable accepted/conditional/rejected decision bound to exact deployed identity with no catalogue widening (Item 10); exact preregistered case/verifier/threshold freeze, customer-environment execution, and business-state/SLO evidence mechanics remain HOR-533/HOR-534. |
...
| Compromised trusted authority | A compromised evaluator/custodian/builder/broker/publisher/qualifier/support authority is detected via durable audit/signature evidence that its outputs diverge from the trusted chain, its authority is revoked and refused at every downstream consumer, and any decision/output it emitted is re-validated or discarded rather than relied on; fail-closed means downstream trust is denied, not that a compromised key somehow cannot act. | Independent authority rotation + Foundry/control-plane detection-and-revocation audit. |
...
| Stale identity across the chain | A stale candidate/source/artifact/publisher/deployment/acceptance reference at any link (candidate → evaluator → source → artifact → publisher → deployment → customer → acceptance → support) fails closed at that link, stops downstream propagation, and triggers a fresh explicit requalification or new candidate; no near-cell evidence is borrowed across any link. | Foundry reference/evidence + release/overlay/customer authorities at each link. |
