...
Owner-unit validation forces CPU and GPU provisioning failures after the cloud resource identity exists, then forces both cleanup paths to fail and proves the retained resource still carries the `forge-e2e` reaper tag. The reaper seam independently proves that expired tagged orphans are deleted, active resources are retained, and one delete failure does not suppress later deletions or report a false-success run.
