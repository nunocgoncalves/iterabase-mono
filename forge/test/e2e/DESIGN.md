...
The path provisions a clean host, installs K3s `v1.34.10+k3s1` directly with `--flannel-backend=none` and `--disable-network-policy`, then installs checksum-pinned Cilium `1.19.7` with its documented K3s cluster-pool IPAM and exact `10.42.0.0/16` pool. Before any candidate artifact is installed it proves Cilium health, sole-CNI identity, and real allow/deny `NetworkPolicy` enforcement, while proving Flannel and embedded kube-router policy state are absent. It then installs checksummed PR #65 charts and source images through Forge's existing chart lifecycle without adding a CNI field to `forge.yaml`.

Acceptance is one pass or one retained blocker: an operationally Ready two-worker managed-RWX AgentPool writes and reads checksummed bytes, then the test deletes only the established share-manager and starts both recovery deadlines when deletion is accepted. Longhorn has 30 seconds to restore successful prior-data and fresh cross-worker RWX I/O on the unchanged workers. If that proof is unavailable at the boundary, the path irrevocably selects AgentPool fallback and requires fresh affected-worker UIDs before the same I/O/integrity proof; unchanged original workers cannot satisfy fallback. Either branch must finish by 60 seconds with the desired replica count unchanged. There is no annotation, scaling/spec mutation, attachment reset, application/business retry policy, repair, pass-on-rerun, or deadline extension. Success and failure artifacts retain the selected branch, exact inputs, accepted-fault/30-second/60-second timestamps, Cilium configuration/endpoints, strict TCP/9503 policy, share-manager attempts/logs, Longhorn transitions, workers, and I/O checksums.
