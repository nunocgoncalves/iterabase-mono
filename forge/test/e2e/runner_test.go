...
func TestE2E(t *testing.T) {
	...
	suite.Add(
		...
		sharede2e.Define(sharede2e.Scenario[*digitalOceanGPUState]{
			Metadata: forgeScenarioMetadata(
				"digitalocean-gpu",
				"Provisions a fresh GPU host and proves GPU substrate plus minimal real serving composition.",
				sharede2e.TierF3,
				[]string{"HOR-481", "HOR-485"},
				[]string{"forge", "iterabase-platform-chart"},
				"test-e2e-gpu", 70, "gpu",
			),
			NewState: newDigitalOceanGPUState,
			Stages: []sharede2e.Stage[*digitalOceanGPUState]{
				{Name: "provision-host", Run: provisionGPUStage},
				...
				{Name: "apply-platform", DependsOn: []string{"assert-gpu-smoke"}, Run: applyInferencePlatformStage},
