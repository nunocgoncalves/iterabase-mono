	...
	suite.Add(
		...
		sharede2e.Define(sharede2e.Scenario[*digitalOceanCPUState]{
			Metadata: forgeScenarioMetadata(
				"digitalocean-cpu",
				"Provisions a fresh CPU host and proves Forge bootstrap, managed Longhorn RWX prerequisites/conformance/persistence, migration, exact source/Flux handoff, secret transport, idempotent reconciliation, diagnostics, and cleanup.",
				sharede2e.TierF3,
				[]string{"HOR-406", "HOR-469", "DES-HOR-424-01", "DES-HOR-469-01"},
		...
		sharede2e.Define(sharede2e.Scenario[*rwxThreeNodeState]{
			...
			Stages: []sharede2e.Stage[*rwxThreeNodeState]{
				{Name: "provision-three-nodes", Run: provisionRWXThreeNodesStage},
				{Name: "bootstrap-k3s-baseline", DependsOn: []string{"provision-three-nodes"}, Run: bootstrapRWXThreeNodeK3sStage},
				{Name: "install-managed-rwx-companion", DependsOn: []string{"bootstrap-k3s-baseline"}, Run: installThreeNodeRWXCompanionStage},
				{Name: "seed-three-replica-volume", DependsOn: []string{"install-managed-rwx-companion"}, Run: seedThreeNodeRWXVolumeStage},
				{Name: "replace-lost-storage-node", DependsOn: []string{"seed-three-replica-volume"}, Run: replaceLostRWXNodeStage},
