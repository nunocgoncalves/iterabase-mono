	...
	suite.Add(
		...
		sharede2e.Define(sharede2e.Scenario[*digitalOceanCPUState]{
			Metadata: forgeScenarioMetadata(
				...
				sharede2e.TierF3,
				[]string{"HOR-406", "HOR-538", "DES-HOR-538-01", "DES-HOR-538-02"},
		...
		sharede2e.Define(sharede2e.Scenario[*digitalOceanCPUState]{
			...
			Stages: []sharede2e.Stage[*digitalOceanCPUState]{
				...
				{Name: "fresh-exact-head-install", DependsOn: []string{"provision-exact-candidate-and-disk"}, Run: cpuDiagnosticStage(failureDomainForgeHandoff, runOverlayStage)},
				...
				{Name: "fill-gate-and-reopen-workspace", DependsOn: []string{"setup-two-worker-rwo-agentpool"}, Run: cpuDiagnosticStage(failureDomainSubstrate, exerciseWorkspaceCapacityGateStage)},
				{Name: "seed-committed-workspace-bytes", DependsOn: []string{"fill-gate-and-reopen-workspace"}, Run: cpuDiagnosticStage(failureDomainSubstrate, seedLocalPathReapplyStage)},
				...
				{Name: "replace-worker-on-unchanged-rwo-claim", DependsOn: []string{"assert-persisted-bytes"}, Run: cpuDiagnosticStage(failureDomainDependentSmoke, replaceWorkspaceWorkerStage)},
			},
			Diagnostics: cpuScenarioDiagnostics(), Cleanup: cpuScenarioCleanup(),
		}),
